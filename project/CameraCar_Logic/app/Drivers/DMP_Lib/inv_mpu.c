 ...
 * min(int a, int b)
 */
#if defined EMPL_TARGET_K60F15
#include "common.h"   
#include "MPU6050_Soft.h"

#define i2c_write     MPU6050_WriteBuffer
#define i2c_read      MPU6050_ReadBuffer 
#define delay_ms      LPLD_SYSTICK_DelayMs
...
static int accel_self_test(long *bias_regular, long *bias_st)
{
    ...
    log_i("DMP: <Info> ACCEL Bias Test:\r\n");
    log_i("DMP: <Info> Initial Biases:%ld, %ld, %ld\r\n", bias_regular[0], bias_regular[1], bias_regular[2]);

    if(result == 0) {
    	if(bias_regular[0]>accel_max_xy_bias) {
    		log_i("DMP: <Info> ACCEL FAIL X\r\n");
    		result |= 1;
    	}
    	if(bias_regular[1]>accel_max_xy_bias) {
    		log_i("DMP: <Info> ACCEL FAIL Y\r\n");
    		result |= 2;
    	}
    	if(bias_regular[2]>accel_max_z_bias) {
    		log_i("DMP: <Info> ACCEL FAIL Z\r\n");
    		result |= 4;
    	}
    }
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
    ...
    log_i("DMP: <Info> GYRO Bias Test:\r\n");
    log_i("DMP: <Info> Initial Biases:%ld, %ld, %ld\r\n", bias_regular[0], bias_regular[1], bias_regular[2]);
    if(result == 0) {
    	if(bias_regular[0]>gyro_max_bias) {
    		log_i("DMP: <Info> GYRO FAIL X\r\n");
    		result |= 1;
    	}
    	if(bias_regular[1]>gyro_max_bias) {
    		log_i("DMP: <Info> GYRO FAIL Y\r\n");
    		result |= 2;
    	}
    	if(bias_regular[2]>gyro_max_bias) {
    		log_i("DMP: <Info> GYRO FAIL Z\r\n");
    		result |= 4;
    	}
    }
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (i2c_read(st.hw->addr, REG_6500_XA_ST_DATA, 3, regs)) {
    	if(debug)
    		log_i("DMP: <Info> Reading OTP Register Error.\n");
    	return 0x07;
    }
    if(debug)
    	log_i("DMP: <Info> Accel OTP:%d, %d, %d\n", regs[0], regs[1], regs[2]);
	for (i = 0; i < 3; i++) {
		if (regs[i] != 0) {
	...
	if(otp_value_zero == 0) {
		...
		if(debug) {
			log_i("DMP: <Info> ACCEL:CRITERIA B\r\n");
			log_i("DMP: <Info> Min MG: %7.4f\r\n", accel_st_al_min/1.f);
			log_i("DMP: <Info> Max MG: %7.4f\r\n", accel_st_al_max/1.f);
		}
	...
	/* Self Test Pass/Fail Criteria C */
		...
		if(debug)
			log_i("DMP: <Info> Accel:CRITERIA C: bias less than %7.4f\n", accel_offset_max/1.f);
		for (i = 0; i < 3; i++) {
			if(fabs(bias_regular[i]) > accel_offset_max) {
				if(debug)
					log_i("DMP: <Info> FAILED: Accel axis:%d = %ld > 500mg\n", i, bias_regular[i]);
				result |= 1 << i;	//Error condition
			}
		}
	}
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
	...
	if(otp_value_zero == 0) {
		if(debug)
			log_i("DMP: <Info> GYRO:CRITERIA A\n");
		...
		for (i = 0; i < 3; i++) {
			...
			if (fabs(st_shift_ratio[i]) < test.max_gyro_var) {
				if(debug)
					log_i("DMP: <Info> Gyro Fail Axis = %d\n", i);
				result |= 1 << i;	//Error condition
			}
		}
	}
		...
		if(debug) {
			log_i("DMP: <Info> GYRO:CRITERIA B\r\n");
			log_i("DMP: <Info> Max DPS: %7.4f\r\n", gyro_st_al_max/1.f);
		}
		...
		for (i = 0; i < 3; i++) {
			...
			if(st_shift_cust[i] < gyro_st_al_max) {
				if(debug)
					log_i("DMP: <Info> GYRO FAIL axis:%d greater than 60dps\n", i);
				result |= 1 << i;	//Error condition
			}
		}
	}
	...
	/* Self Test Pass/Fail Criteria C */
		...
		if(debug)
			log_i("DMP: <Info> Gyro:CRITERIA C: bias less than %7.4f\n", gyro_offset_max/1.f);
		for (i = 0; i < 3; i++) {
			if(fabs(bias_regular[i]) > gyro_offset_max) {
				if(debug)
					log_i("DMP: <Info> FAILED: Gyro axis:%d = %ld > 20dps\n", i, bias_regular[i]);
				result |= 1 << i;	//Error condition
			}
		}
	}
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    if(debug)
    	log_i("DMP: <Info> Starting Bias Loop Reads\n");
    ...
    if(debug) {
    	log_i("DMP: <Info> Accel offset data HWST bit=%d: %7.4f %7.4f %7.4f\r\n", hw_test, accel[0]/65536.f, accel[1]/65536.f, accel[2]/65536.f);
    	log_i("DMP: <Info> Gyro offset data HWST bit=%d: %7.4f %7.4f %7.4f\r\n", hw_test, gyro[0]/65536.f, gyro[1]/65536.f, gyro[2]/65536.f);
    }
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
    ...
    if(debug)
    	log_i("DMP: <Info> Starting MPU6500 HWST!\r\n");
    ...
    if(debug)
    	log_i("DMP: <Info> Retrieving Biases\r\n");
    ...
    if (ii == tries) {
         ...
         */
        if(debug)
        	log_i("DMP: <Info> Retrieving Biases Error - possible I2C error\n");
    ...
    if(debug)
    	log_i("DMP: <Info> Retrieving ST Biases\n");
    ...
    if (ii == tries) {
        ...
        if(debug)
        	log_i("DMP: <Info> Retrieving ST Biases Error - possible I2C error\n");
    ...
    if(debug)
    	log_i("DMP: <Info> Accel Self Test Results: %d\n", accel_result);

    gyro_result = gyro_6500_self_test(gyro, gyro_st, debug);
    if(debug)
    	log_i("DMP: <Info> Gyro Self Test Results: %d\n", gyro_result);
    ...
    if(debug)
    	log_i("DMP: <Info> Compass Self Test Results: %d\n", compass_result);
    if (!compass_result)
        result |= 0x04;
#else
    result |= 0x04;
...
static int setup_compass(void)
{
    ...
    if (akm_addr > 0x0F) {
        ...
        log_i("DMP: <Error> Compass not found.\n");
        return -1;
    }
