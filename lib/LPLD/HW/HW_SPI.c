...
uint8 LPLD_SPI_Init(SPI_InitTypeDef spi_init_structure)
{
  ...
  //检测参数
  ASSERT(spi_mode <= SPI_MODE_SLAVE);
  ...
  if(spix == SPI0)
  {
    ...
    if(sck_pin == PTA15)
    {
      PORTA->PCR[15] = 0 | PORT_PCR_MUX(2) | PORT_PCR_DSE_MASK;//SCK
		...
		else if(sck_pin == PTD1)
    {
      PORTD->PCR[1] = 0 | PORT_PCR_MUX(2) | PORT_PCR_DSE_MASK;//SCK
    }
    else //sck_pin == PTC5
    {
